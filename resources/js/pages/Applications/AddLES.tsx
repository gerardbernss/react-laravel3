...
export default function AddApplicant() {
    ...
    // ==================== HELPER FUNCTION ====================
    // Function to load all permanent address data in sequence
    const loadPermanentAddressData = async () => {
        ...
        try {
            ...
            if (selectedPresentProvince) {
                // Load cities
                const citiesRes = await axios.get<PsgcItem[]>(
                    `https://psgc.gitlab.io/api/provinces/${selectedPresentProvince}/cities-municipalities`,
                );
                ...
                if (selectedPresentCity) {
                    // Load barangays
                    const barangaysRes = await axios.get<PsgcItem[]>(
                        `https://psgc.gitlab.io/api/cities-municipalities/${selectedPresentCity}/barangays`,
                    );
    ...
    // 2️⃣ Load provinces when PRESENT region changes
    useEffect(() => {
        if (!selectedPresentRegion) return;
    ...
    // 3️⃣ Load cities/municipalities when PRESENT province changes
    useEffect(() => {
    ...
    // 4️⃣ Load barangays when PRESENT city changes
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        // 1. ✅ FIX: Immediately copy the dropdown options
        // This ensures the Select component has the list needed to find the value
        ...
        // ✅ Add these dependencies so it updates if the present address changes
        presentRegions,
        presentProvinces,
        presentCities,
        presentBarangays,
    ]);
    // 5️⃣ Load provinces when PERMANENT region changes
    useEffect(() => {
        if (isSameAddress) return;
        ...
        // Only clear if not synced with present address
        if (!isSameAddress) {
            setPermanentCities([]);
    ...
    // 6️⃣ Load cities/municipalities when PERMANENT province changes
    useEffect(() => {
        if (isSameAddress) return;
    ...
    // 7️⃣ Load barangays when PERMANENT city changes
    useEffect(() => {
        if (isSameAddress) return;
        ...
        <div className="bg-[#f5f5f5]">
            ...
            <header className="bg-[#073066] text-white shadow-md">
                <div className="mx-auto flex max-w-[1800px] items-center justify-between px-10 py-6">
                    ...
                    {/* Rightmost Text */}
                    <p className="text-2xl whitespace-nowrap text-white">LES Online Application</p>
                </div>
            </header>
                                                                        ...
                                                                        render={({ field: noteField }) => (
                                                                            <FormItem>
                                                                                <FormControlLabel
                                                                                    ...
                                                                                    label="With a physician’s recommendation certifying that the student is fit to attend school, along with a medical certificate issued within the last two years."
                                                                                    sx={{
                                                                                        alignItems: 'center',
                                                                                        '& .MuiFormControlLabel-label': {
                                                                                            fontSize: '0.875rem',
