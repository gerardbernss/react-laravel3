...
import { type Assessment, useAssessmentShow } from '@/hooks/useAssessmentShow';
                    ...
                    <div className="space-y-4 lg:order-last lg:col-span-2">
                        {assessment.student && (
                            <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                                <h2 className="mb-3 text-sm font-semibold uppercase text-gray-500">Student Information</h2>
                                <div className="grid grid-cols-2 gap-3 text-sm">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-medium text-gray-900">{assessment.student.student_id ?? '—'}</p>
                                    </div>
                                    <div>
                                        <p className="text-gray-500">Grade Level</p>
                                        <p className="font-medium text-gray-900">{assessment.student.grade_level ?? '—'}</p>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                            <h2 className="mb-3 text-sm font-semibold uppercase text-gray-500">Fee Assessment</h2>
                            ...
                            <div className="mt-4 grid grid-cols-3 gap-3 rounded-lg bg-gray-50 p-3 text-center text-sm">
                                <div>
                                    ...
                                    <p className="font-semibold text-gray-900">{formatCurrency(assessment.net_amount)}</p>
                                </div>
                                <div>
                                    <p className="text-gray-500">Total Paid</p>
                                    <p className="font-semibold text-green-700">{formatCurrency(assessment.total_paid)}</p>
                                </div>
                                <div>
                                    <p className="text-gray-500">Balance</p>
                                    <p className={`text-lg font-bold ${assessment.balance > 0 ? 'text-red-600' : 'text-green-600'}`}>
                            ...
                            <div className="mt-4 rounded-lg border border-blue-100 bg-blue-50 p-3 text-sm">
                                ...
                                {editingMinimum && (
                                    <form onSubmit={handleSaveMinimum} className="mt-2 flex items-center gap-2">
                                        <Input
                                            type="number"
                                            step="0.01"
                                            min="0"
                        ...
                        <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
                            <div className="border-b px-4 py-3">
                                <h2 className="text-sm font-semibold uppercase text-gray-500">Payment History</h2>
                            </div>
                                            ...
                                            {assessment.payments.map((p) =>
                                                editingId === p.id ? (
                                                            ...
                                                            >
                                                                <div>
                                                                    ...
                                                                    {editErrors.payment_date && (
                                                                        <p className="mt-0.5 text-xs text-red-500">{editErrors.payment_date}</p>
                                                                    )}
                                                                </div>
                                                                <div>
                                                                ...
                                                                <div>
                                                                    ...
                                                                    {editErrors.amount_paid && (
                                                                        <p className="mt-0.5 text-xs text-red-500">{editErrors.amount_paid}</p>
                                                                    )}
                                                                </div>
                                                                <div>
                                                    ...
                                                    <tr key={p.id} className="hover:bg-gray-50">
                                                        ...
                                                        <td className="px-4 py-2 text-gray-400">{p.reference_number ?? '—'}</td>
                                                        <td className="px-4 py-2 text-right font-semibold text-green-700">{formatCurrency(p.amount_paid)}</td>
                    ...
                    <div className="lg:order-first">
                        {isPaid ? (
                            <div className="rounded-lg border-2 border-green-200 bg-green-50 p-6 text-center shadow-sm">
                                ...
                                <p className="mt-1 text-sm text-green-600">This assessment has been fully settled.</p>
                            </div>
                        ) : (
                            <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                                ...
                                <div className="mb-4 space-y-2">
                                    <div className="rounded-lg bg-amber-50 p-3 text-center">
                                        ...
                                        <p className="text-2xl font-bold text-amber-800">{formatCurrency(assessment.balance)}</p>
                                    </div>
                                    {assessment.total_paid < assessment.minimum_amount && (
                                        <div className="rounded-lg bg-blue-50 p-2 text-center">
                                            <p className="text-xs text-blue-600">
                                                Min. to enroll: <span className="font-semibold">{formatCurrency(assessment.minimum_amount)}</span> · needs{' '}
                                                {formatCurrency(assessment.minimum_amount - assessment.total_paid)} more
                                            </p>
                                        </div>
                                    )}
                                </div>
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    ...
                                    <div>
                                        <label className="mb-1 block text-sm font-medium text-gray-700">
                                            Reference / OR No. <span className="text-gray-400">(optional)</span>
                                        </label>
                                        <Input
                                    ...
                                    {data.amount_paid && (() => {
                                        ...
                                        return (
                                            <div
                                                className={`rounded-md p-2 text-xs ${willFullyPay ? 'bg-green-50 text-green-700' : willEnroll ? 'bg-blue-50 text-blue-700' : 'bg-amber-50 text-amber-700'}`}
                                            >
                                                {willFullyPay ? (
                                    ...
                                    <Button type="submit" className="w-full" disabled={processing || !data.amount_paid}>
