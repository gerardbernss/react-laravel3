...
import { ROLE_COLUMNS, useRoleTable, type ColumnKey } from '@/hooks/useRoleTable';
...
function RoleTableRow({ role, visibleColumns, isSelected, processing, onSelect, onDelete }: RoleTableRowProps) {
...
/** Admin roles list with search, sort, delete, and bulk-delete actions. */
export default function Index() {
    const { flash, roles } = usePage<RolesPageProps>().props;
    const { hasPermission } = usePermissions();
    const {
        hideAlert, setHideAlert,
        processing,
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className={`space-y-6 ${PAGE_PADDING}`}>
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className={PAGE_TITLE}>Roles and Permissions</h1>
                            ...
                            <Popover>
                                ...
                                <PopoverContent className="w-48" align="end">
                                    <div className="space-y-2">
                                        ...
                                        {[...ROLE_COLUMNS, { key: 'actions' as const, label: 'Actions' }].map((col) => (
                                            <div key={String(col.key)} className="flex items-center space-x-2">
                                                <input
                                                    ...
                                                    className="h-4 w-4 rounded border-gray-300"
                                                />
                                                <label htmlFor={String(col.key)} className="text-sm">
                                                    {col.label}
                                                </label>
                                            </div>
                                        ))}
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="sticky top-0 z-10 bg-gray-50">
                                    <tr>
                                        ...
                                        {ROLE_COLUMNS.filter((col) => visibleColumns.includes(col.key)).map((col) => (
                                            <th
                                                key={String(col.key)}
                                                onClick={() =>
                                                    !['permissions', 'users'].includes(col.key) && handleSort(col.key)
                                                }
                                                className={`${TABLE_HEADER_CELL} ${!['permissions', 'users'].includes(col.key) ? 'cursor-pointer hover:bg-gray-100' : ''}`}
                                            >
