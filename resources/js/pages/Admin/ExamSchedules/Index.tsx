...
type SortKey = 'name' | 'exam_date' | 'room' | 'assigned_count' | 'status';
...
export default function Index({ schedules, rooms }: Props) {
    ...
    const sortedItems = useMemo(() => {
        ...
        return [...filteredItems].sort((a, b) => {
            ...
            else if (sortConfig.key === 'exam_date') { aVal = a.exam_date; bVal = b.exam_date; }
            else if (sortConfig.key === 'room') { aVal = a.examination_room?.name ?? ''; bVal = b.examination_room?.name ?? ''; }
            else if (sortConfig.key === 'assigned_count') { aVal = String(a.assigned_count); bVal = String(b.assigned_count); }
                    ...
                    <div className="flex flex-wrap items-end gap-3">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="h-10 rounded-lg border border-gray-300 px-3 text-sm focus:outline-none"
                            />
                        </div>
                        ...
                        <table className="w-full text-sm">
                            <thead className="sticky top-0 z-10 bg-gray-50">
                                <tr>
                                    <th className="cursor-pointer px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 hover:bg-gray-100" onClick={() => toggleSort('name')}>
                                        Schedule <SortIcon col="name" />
                                    </th>
                                    <th className="cursor-pointer px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 hover:bg-gray-100" onClick={() => toggleSort('exam_date')}>
                                        Date &amp; Time <SortIcon col="exam_date" />
                                    </th>
                                    <th className="cursor-pointer px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 hover:bg-gray-100" onClick={() => toggleSort('room')}>
                                        Room <SortIcon col="room" />
                                    </th>
                                    <th className="cursor-pointer px-4 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-500 hover:bg-gray-100" onClick={() => toggleSort('assigned_count')}>
                                        Assigned <SortIcon col="assigned_count" />
                                    </th>
                            ...
                            <tbody>
                                {paginatedItems.length === 0 ? (
                                    ...
                                    return (
                                        <tr key={schedule.id} className="border-b border-gray-200 transition-all hover:bg-slate-50">
                                            ...
                                            <td className="px-4 py-3">
                                                <div className="flex justify-center gap-1">
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={() => setDeleteDialog({ open: true, id: schedule.id, name: schedule.name })}
                                                        disabled={processing}
